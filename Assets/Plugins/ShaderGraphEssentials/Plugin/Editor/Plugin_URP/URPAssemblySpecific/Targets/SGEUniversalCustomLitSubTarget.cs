   ...
   sealed class SGEUniversalCustomLitSubTarget : SGEUniversalSubTarget, ILegacyTarget
    {
        ...
        public override void Setup(ref TargetSetupContext context)
        {
            ...
            // Process SubShaders
            context.AddSubShader(PostProcessSubShader(SubShaders.LitComputeDotsSubShader(target, target.renderType.ToString(), target.renderQueue.ToString())));
            context.AddSubShader(PostProcessSubShader(SubShaders.LitGLESSubShader(target, target.renderType.ToString(), target.renderQueue.ToString())));
        }
...
#region SubShader
        static class SubShaders
        {
            ...
            public static SubShaderDescriptor LitComputeDotsSubShader(SGEUniversalTarget target, string renderType, string renderQueue)
            {
            ...
            public static SubShaderDescriptor LitGLESSubShader(SGEUniversalTarget target, string renderType, string renderQueue)
            {
                // SM 2.0, GLES
...
#region Passes
        static class LitPasses
        {
            ...
            public static PassDescriptor Forward(SGEUniversalTarget target, PragmaCollection pragmas = null)
            {
            ...
            public static PassDescriptor GBuffer(SGEUniversalTarget target)
            {
                var result = new PassDescriptor
                {
                    // Definition
